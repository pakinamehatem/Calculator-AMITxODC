...
int main(void)
{
	...
	while (1)
	{
		...
		if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6))  //pin6 minus
		{
			minus = ((n1%16) - ((n1-(n1%16))/16));
			digit[1]=minus/10;
			digit[0]=minus%10;
		}

		else if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_7))  //pin 7 summation
	{
		sum = ((n1%16) + ((n1-(n1%16))/16));
		digit[1]=sum/10;
		digit[0]=sum%10;
	}
		else if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_8)) //pin 8 multiplication
	{
		mult = ((n1%16) * ((n1-(n1%16))/16));
		digit[1]=mult/10;
		digit[0]=mult%10;
	}
		else if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9)) //pin 9 division
	{
		div = ((n1%16) / ((n1-(n1%16))/16));
		digit[1]=div/10;
		digit[0]=div%10;
	}
		//display result
